...
interface Props {
    plumber_imageUrl:string | null;
    plumber_rating:number | null;
    plumber_category:string | null;
    plumber_name:string | null;
    plumber_phone:string | null;
    plumber_address:string | null;
}

export const CardSliderProfessional = ({ plumber_imageUrl,plumber_rating,plumber_category,plumber_name,plumber_phone,plumber_address } : Props ) => {

    return (
        <div className="flex flex-col gap-y-2 rounded group bg-white">
            <div className="relative overflow-hidden rounded">
                <Link href={`/professional/${plumber_name}`} title={plumber_name ? plumber_name : 'plumber'} className="rounded">
                    <Image priority={ false } src={plumber_imageUrl ? plumber_imageUrl : '/images/placeholder_image.webp'} alt={plumber_name ? plumber_name : 'plumber'} width={0} height={0} sizes="100vw" className="relative w-full h-full rounded transition-transform duration-500 hover:scale-125 aspect-square object-cover" />
                </Link>
                <div className="absolute top-0 left-0 flex items-center gap-x-2 bg-yellow-site text-slate-100 py-1 px-2 rounded-br">
                    ...
                    <span className="font-semibold">Our Rating: {plumber_rating ? plumber_rating : 0}</span>
                </div>
                <Link href={'/plumbers'} className="absolute bottom-0 right-0 flex items-center gap-x-2 bg-blue-site text-slate-100 py-1 px-2 rounded-tl">
                    ...
                    <span>{plumber_category ? plumber_category : 'Uncategorized'}</span>
                </Link>
            </div>
